...
Public Class ExportBillingForm
    ...
    Private Sub ExportForm_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select custCode from custTab order by custcode", Conn)
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd1 As New SqlCommand("select ItemCode from ItemTab order by ItemCode", Conn)
    ...
    Sub SaveRecord()
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select max(Billno) from SaMainTab", Conn)
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        q1var = "insert into SaMainTab("
        q2var = " values("
        q1var = q1var & "billNo" & ","
        q2var = q2var & "" & BillNoVar & ","
        q1var = q1var & "BDate" & ","
        q2var = q2var & "'" & Format(bDate.Value, "dd/MMM/yyyy") & "',"
    ...
    Sub disRecords()
        ...
        Dim adp As New SqlDataAdapter("Select Billno,PartyCode From SaMainTab where btype = 4 order by Billno", Conn)
        adp.Fill(DS1)
    ...
    Private Sub DG1_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DG1.CellContentClick
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select * from SaMainTab where billNo=" & pkVar & "", Conn)
        Dim D1 As SqlDataReader = Cmd0.ExecuteReader()
        If D1.HasRows Then
            D1.Read()
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd4 As New SqlCommand("select * from SaDetTab where billNo=" & pkVar & "", Conn)
    ...
    Private Sub butModify_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles butModify.Click
        ...
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim cmd1 As New SqlCommand("Delete from SaMainTab where billNo=" & pkVar & "", Conn)
        cmd1.ExecuteNonQuery()
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim cmd2 As New SqlCommand("Delete from SaDetTab where billNo=" & pkVar & "", Conn)
    ...
    Private Sub ComboBox2_Leave(ByVal sender As Object, ByVal e As System.EventArgs) Handles ComboBox2.Leave
        If Conn.State = ConnectionState.Open Then Conn.Close()
        Conn.Open()
        Dim Cmd0 As New SqlCommand("select sPrice,TaxPer from ItemTab where ItemCode='" & ComboBox2.Text & "'", Conn)
        Dim D1 As SqlDataReader = Cmd0.ExecuteReader()
        If D1.HasRows Then
            D1.Read()
        ...
        PictureBox1.Image = Image.FromFile("IMAGES/" & ComboBox2.Text & ".bmp")
    End Sub
    ...
    Private Sub PrintDocument1_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ...
        e.Graphics.DrawString("Organic Food And Co", MyFont, Brushes.Black, XPos, YPos)
        YPos += 50
        MyFont = New Font("Arial", 14)
        XPos = 10
        e.Graphics.DrawString("Cash Bill", MyFont, Brushes.Black, XPos, YPos)
        XPos = 200
        ...
        e.Graphics.DrawString("To", MyFont, Brushes.Black, XPos, YPos)
        YPos += 25
        XPos = 10
        ...
        e.Graphics.DrawString("Total Amt : ", MyFont, Brushes.Black, XPos, YPos)
        XPos = 650
        e.Graphics.DrawString(TxtT3.Text, MyFont, Brushes.Black, XPos, YPos)
